...
class Cipher(object):
	...
	if mapping == "reverse":
	    ...
	    key = self.alphabet[13:] + self.alphabet[:13]
        else:
            key = self.alphabet[:14][::-1] + self.alphabet[14:][::-1]
    ...
    def isvalid(self, message):
        ...
        """
        uppercases = self.alphabet.upper()
        for i in message:
            if i not in self.alphabet and i not in uppercases:
                return False
        return True

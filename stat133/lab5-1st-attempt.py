...
def simulate_grades(class_size, max_scores=[100,100,100]):
    ...
    return 100 * rand(class_size, len(max_scores))
...
def simulate_grade_df(class_size, grade_items={'F':100,'M':100,'HW':10}):
    ...
    for key in grade_items: 
        max_scores.append(grade_items[key])
    values = simulate_grades(class_size, max_scores)
    for key in grade_items:
        labels.append(key)
    return DataFrame(values, columns=labels)

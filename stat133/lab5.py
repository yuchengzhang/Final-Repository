...
def simulate_grades(class_size, max_scores=[100,100,100]):
    ...
    ----------
    class_size : int 
        Number of student grade records to simulate
    ...
    True
    """
    grade_arr = rand(class_size, len(max_scores))
    score_weighter = array(max_scores)
    return grade_arr * score_weighter
...
def simulate_grade_df(class_size, grade_items={'F':100,'M':100,'HW':10}):
    ...
    True
    """
    grade_list = [pd.Series(simulate_grades(class_size, [grade_items[key]]).transpose().flatten(), name=key) 
                  for key in grade_items]
    grade_df = DataFrame(grade_list)
    return grade_df.T
